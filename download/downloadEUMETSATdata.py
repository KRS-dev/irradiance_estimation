...
if __name__ == '__main__':
    ...
    for day in pd.date_range(start, end, freq='D'):
        ...
        obs.date = midday.strftime('%Y/%m/%d %H:%M:%S') # pyephem requires datetimes in string utc format
        sunrise = obs.previous_rising(ephem.Sun()).datetime()
        sunset = obs.next_setting(ephem.Sun()).datetime()


        day_products[datetime.strftime(day, '%Y%m%d')] = \
            selected_collection.search(
                dtstart=sunrise,
                dtend=sunset)

    print("Number of Images: ", sum([products.total_results for products in day_products.values()]))
    ...
    for day_str, products in tqdm(day_products.items()):
        ...
        print(len(products))
        part_download_product= partial(download_product, SAVE_PATH=SAVE_PATH, verbose=args.verbose)

        with ThreadPoolExecutor(max_workers=10) as executor:
            if args.verbose:
                results = list(tqdm(executor.map(part_download_product, products), total=N))
            else:
