...
import torch
from utils.plotting import latlon_error_plot, prediction_error_plot
from utils.plotting import SZA_error_plot, dayofyear_error_plot

class LitConvResNet(L.LightningModule):
    def __init__(self, 
                 learning_rate, 
                 config 
                 parameter_loss=False, 
                 alpha=0.1, 
                 zero_loss = None,
    ...
    def setup(self, stage):
        ...
        self.other_metrics = MetricCollection([MeanMetric().to(self.device), 
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        self.y[dataloader_idx].append(y)
        self.y_hat[dataloader_idx].append(y_hat)
        self.x_attr[dataloader_idx].append(batch[1])
        return loss
    ...
    def test_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        y_hat = self.forward(X, x)
        y_hat[y_hat < -1] = -1
        loss = self.valid_metric[0](y_hat, y)
        self.log("test_loss", self.valid_metric[0], on_epoch=True, logger=True, sync_dist=True)
        self.y.append(y)
    ...
    def on_validation_epoch_end(self):
        for i in range(self.num_dataloaders):
            y = torch.cat(self.y[i])
            y_hat = torch.cat(self.y_hat[i])
            x_attr = torch.cat(self.x_attr[i])
    ...
    def on_validation_epoch_start(self):
        ...
        self.y_hat = [[] for _ in range(self.num_dataloaders)]
        self.x_attr = [[] for _ in range(self.num_dataloaders)]   
