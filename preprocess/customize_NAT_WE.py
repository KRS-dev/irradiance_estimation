...
if __name__ == "__main__":
    ...
    for ch in chains:
        ...
        for dt in tqdm(
            daterange,
            desc="Looking for input data and checking previously customized data.",
        ):
            if aggregate_time == "day":
            ...
            finished_customizations = glob(
                os.path.join(SAVE_PATH, f"{ch.product}_{dt_str}_*.nc")
            )
            ...
            if len(customized) > 0:
                results_dt_dict = {
                    x.split("-NA-")[-1].split(".")[0]: x for x in results_dt
                }
                customized_dtstr = set(
                    [
                        x.split("Z")[0].split("_")[-1].replace("T", "")
                        for x in customized
                    ]
                )
                ...
                if ch.product == "HRSEVIRI_HRV":
                    ...
                    customized_dt_corrected = [
                        datetime.strptime(x, "%Y%m%d%H%M%S") + offset
                        for x in customized_dtstr
                    ]
                    customized_dtstr = set(
                        [x.strftime("%Y%m%d%H%M") for x in customized_dt_corrected]
                    )  # Remove seconds as they differ as well
                    results_dt_dict = {
                        k[:-2]: v for k, v in results_dt_dict.items()
                    }  # same

                results_dtstr = set(results_dt_dict.keys())
                a = (
                    results_dtstr - customized_dtstr
                )  # #FIXED with 12min offset, doesnt work because sensing time of HRSEVIRI_HRV is later than HRSEVIRI or the time in the zip file
        ...
        uncompressed_dict = {}
        for dt_str in tqdm(
            dt_ls_running, desc=f"{ch.product} Combining per {args.aggregatetime}."
        ):
            uncompressed_fn = combine_netcdf_files(
                SAVE_PATH, dt_str, ch.product, verbose=args.verbose
            )
            if args.verbose:
                print(f"{dt_str} {ch.product} Combined the .nc files.")
            compressed_fn = os.path.join(
                SAVE_PATH, ch.product + "_" + dt_str + "_FPC.nc"
            )
        ...
        w = tqdm(
            ...
            desc=f"{ch.product} Compressing files per {args.aggregatetime}.",
        )
