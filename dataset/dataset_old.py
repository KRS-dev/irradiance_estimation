        ...
        if stage == 'fit':
            ...
            if self.patch_size is not None:
                with benchmark('patch train'):
                    self.msg_train = MSGDatasetPatched(msgdataset=self.msg_train0, patch_size=self.patch_size)
                    self.msg_train.load()
                with benchmark('patch validation'):
                    self.msg_validation = MSGDatasetPatched(msgdataset=self.msg_validation0, patch_size=self.patch_size)
