...
def chunk_zarr(zarr_store, patch_size, batch_size):
    ...
    x_vars = set(full_ds.variables) - {'lat_bnds', 'lon_bnds', 'lat','lon','time','SIS', 'record_status'} 
    for var in x_vars:
    ...
    half_ds = full_ds.sel(time=slice(datetime(2015,1,1,0,0), None)) # 2014 is used as test
    test_ds = full_ds.sel(time=slice(datetime(2014,1,1,0,0,0), datetime(2015,1,1,0,0)))
    ...
    train_ds.chunk({'time':batch_size, 'lat':patch_size[1], 'lon':patch_size[0]}).to_zarr( '/scratch/snx3000/kschuurm/DATA/train.zarr', mode='w', safe_chunks=True)
    valid_ds.chunk({'time':batch_size, 'lat':patch_size[1], 'lon':patch_size[0]}).to_zarr( '/scratch/snx3000/kschuurm/DATA/valid.zarr', mode='w', safe_chunks=True)
    test_ds.chunk({'time':batch_size, 'lat':patch_size[1], 'lon':patch_size[0]}).to_zarr( '/scratch/snx3000/kschuurm/DATA/test.zarr', mode='w', safe_chunks = True)
    print('done')
