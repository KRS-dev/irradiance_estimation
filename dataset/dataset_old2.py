...
class MSGDataset(Dataset):
    ...
    def __getitem__(self, idx):
        if self.ds is not None:
            xarr = (
                self.ds[self.x_vars]
                .isel(time=idx)[["time", "lon", "lat"]]
                .to_array()
                .squeeze(dim="variable")
            )  # select time slice, reshape to t lon lat
            yarr = (
                self.ds[self.y_var]
                .isel(time=idx)[["time", "lon", "lat"]]
                .to_array()
...
class MSGDatasetBatched(Dataset):
    def __init__(
        self, generator, y_var=None, x_vars=None, transform=None, target_transform=None
    ):
    ...
    def __getitem__(self, idx) -> Tuple[Any, Any]:
        ...
        X_batch = torch.tensor(
            a[self.x_vars].to_array().values
        )  # , names=('B','C','T', 'X', 'Y'))
        y_batch = torch.tensor(a[self.y_var].values)  # , names=('B', 'T', 'X', 'Y'))

        X_batch = X_batch.squeeze(
            -2
        )  # Squeeze out time dimension which is always second to last
    ...
    def setup(self, stage: str):
        if stage == "fit":
            with benchmark("initialize MSGdataset full test"):
                ...
                self.msg_full.ds = self.msg_full.ds.dropna(
                    dim="time", how="all", subset=["SIS"]
                )

            with benchmark("train_test_split"):
                train_ds = self.msg_full.ds.sel(
            ...
            if self.patch_size is not None:
                ...
                with benchmark("set up batch generator for patches"):
                    traingenerator = BatchGenerator(
                        ds=self.msg_train_ds,
                        input_dims={
                            "time": 1,
                    ...
                    validgenerator = BatchGenerator(
                        ds=self.msg_validation_ds,
                        input_dims={
                            "time": 1,
                            "lat": self.patch_size[1],
                            "lon": self.patch_size[0],
                    ...
                    self.msg_train = MSGDatasetBatched(
                        generator=traingenerator, y_var="SIS"
                    )
                    self.msg_test = MSGDatasetBatched(
                        generator=validgenerator, y_var="SIS"
                    )
            else:
        ...
        if stage == "test":
            test_ds = self.msg_full.ds.sel(
                time=slice(datetime(2014, 1, 1, 0, 0), datetime(2014, 12, 31, 23, 59))
            )
            if self.patch_size is not None:
                testgenerator = BatchGenerator(
                    ds=test_ds,
...
if __name__ == "__main__":
    with Client() as client:
        ...
        dm = MSGDataModule(
            zarr_store=file, batch_size=6, patch_size=(128, 128), num_workers=0
        )
        dm.setup("fit")
