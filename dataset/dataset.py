...
class MSGDataset(Dataset):
    def __init__(self, dataset, y_var, x_vars=None, transform=None, target_transform=None):
...
class MSGDatasetBatched(MSGDataset):
    def __init__(self, zarr_store, 
        ...
        super().__init__(zarr_store, y_var, x_vars, transform=transform, target_transform=target_transform)
...
class MSGDatasetPoint(MSGDatasetBatched):
    ...
    def __getitem__(self, idx) -> Tuple[Any, Any]:
        ...
        X_batch = X_batch.fillna(-99999) # for training we do not want any nans. Very negative values should cancel with ReLU 
    ...
    def get_xarray_batch(self, idx) -> Tuple[xarray.DataArray, xarray.DataArray]:
        ...
        if self.patch_size[0] % 2 == 0:
            ...
            y_batch_point = y_batch.isel(lat=slice(idx[0], idx[1]+1), lon=slice(idx[0], idx[1]+1)).mean(dim=('lat','lon'), keepdims=True).to_dataarray(dim='channels')
        else:
...
class MSGDataModule(L.LightningDataModule):
    def __init__(self, batch_size: int = 32, patch_size: (int, int)=None, num_workers:int=12, x_vars=None, transform=None):
        super().__init__()
        self.batch_size = batch_size
        self.patch_size = patch_size
...
class MSGDataModulePoint(MSGDataModule):
    def __init__(self, *args, **kwargs):
        ...
        assert self.patch_size is not None, 'To create a patch to point datamodule a patchsize is required'
        

    def setup(self, stage: str ):
        if stage == 'fit':
            # Using xbatcher to make batches of patched data
            self.train_dataset = MSGDatasetPoint('/scratch/snx3000/kschuurm/DATA/train.zarr', 
                                                    y_var='SIS', 
                                                    patch_size=self.patch_size, batch_size=self.batch_size)
            self.val_dataset = MSGDatasetPoint('/scratch/snx3000/kschuurm/DATA/valid.zarr', 
                                                    y_var='SIS', patch_size=self.patch_size, 
                                                    batch_size=self.batch_size)
        if stage == 'test':
            self.test_dataset = MSGDatasetBatched('/scratch/snx3000/kschuurm/DATA/validation.zarr',
...
if __name__ == '__main__':
    ...
    ds = MSGDatasetPoint('/scratch/snx3000/kschuurm/DATA/valid.zarr', y_var='SIS', x_vars=['channel_1', 'channel_2'], x_features_bool=True, patch_size=(15,15), transform=MinMax.forward_dataset)
