...
def train_test_split(msgdataset, timeindex):
    ...
    for month in month_dr:
        ...
        slice_end = last_day_of_month(slice_start) + timedelta(hours=23, minutes=59, seconds=59)
...
class MSGDataset(Dataset):
    def __init__(self, train_paths, sarah_paths):
        ...
        a = sorted(list(set(self.x.time.dt.round('30min').values).intersection(set(self.sarah.time.dt.round('30min').values))))
        
        self.sarah = self.sarah.reindex(time=a, lat=self.x.lat, lon=self.x.lon, method='nearest')
...
class MSGDataModule(L.LightningDataModule):
    ...
    def setup(self, stage:str):
        ...
        self.msg_full = MSGDataset('/scratch/snx3000/kschuurm/DATA/customized/HRSEVIRI_2015*', '/scratch/snx3000/kschuurm/DATA/SARAH3/SIS_2015.nc')
        self.msg_test = MSGDataset('/scratch/snx3000/kschuurm/DATA/customized/HRSEVIRI_2014*', '/scratch/snx3000/kschuurm/DATA/SARAH3/SIS_2014.nc')

        self.msg_train, self.msg_validation = train_test_split(self.msg_full, self.msg_full.output.indexes['time'])
